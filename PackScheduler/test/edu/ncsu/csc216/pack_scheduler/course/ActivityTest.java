...
/**
 * A test for the activity class, only checks the check conflict method other tests are in course and event
 * @author James Ticatic
 */
public class ActivityTest {

	/**
	 * Test method for checking conflicts
	 */
	@Test
	public void testCheckConflict() {
	  ...
	  //Update a1 with the same meeting days and a start time that overlaps the end time of a2
	    ...
	    a1.setMeetingDays("THF");
	    a1.setActivityTime(1400, 1515);
	    a2.setMeetingDays("MWF");
	    a2.setActivityTime(1330, 1420);
	    try {
	        a1.checkConflict(a2);
	        fail(); //ConflictException should have been thrown, but was not.
	    } catch (ConflictException e) {
	        //Check that the internal state didn't change during method call.
	        assertEquals("THF 2:00PM-3:15PM", a1.getMeetingString());
	        assertEquals("MWF 1:30PM-2:20PM", a2.getMeetingString());
	    }
	}
}
